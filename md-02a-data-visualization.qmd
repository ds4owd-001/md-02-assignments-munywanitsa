...
3.  Do not display (ignore) the outliers in the plot. **Note: Use a search engine or an AI tool to find the solution and add the link to the solution you have found.**
    ...
    [https://www.r-bloggers.com/2020/01/how-to-remove-outliers-in-r/https://www.r-bloggers.com/2020/01/how-to-remove-outliers-in-r/](https://www.r-bloggers.com/2020/01/how-to-remove-outliers-in-r/)
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: ***It makes the height of the bar proportional to the number of cases in each group***
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: ***I observed that the first number in the breaks argument is the starting value of the histogram, the second value if the ending value of the histogram and the third number represents the size of the bars in the histogram***
...
ggplot(data = gapminder_2007,
       mapping = aes(x = lifeExp,
                     fill = continent)) +
  geom_histogram(col = "grey30",breaks = seq(36, 100, 2.5))
```
